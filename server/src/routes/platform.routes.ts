...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [userCount, salespersonCount, customerCount] = await Promise.all([
      prisma.user.count({ where: { tenantId: tenant.id } }),
      prisma.salesperson.count({ where: { tenantId: tenant.id } }),
      prisma.customer.count({ where: { tenantId: tenant.id } }),
    ]);
    res.json({ ...tenant, userCount, salespersonCount, customerCount });
  })
);
...
// Manual billing path: until a real payment provider (Razorpay/Stripe) is wired in via
// services/billing.ts, a platform admin assigns/changes a tenant's plan and status directly.
// Real provider integration will call the same subscription update from a verified webhook
// instead of this endpoint - the shape of what gets written doesn't change.
router.patch(
